...
export class DateRangepickerComponent implements OnInit {
  ...
  days: IDay[];
  range: string;
  defaultOptions: DateRangePickerOptions = {
    labels: ['Start', 'End'],
    locale: 'en',
    menu: [],
  ...
  getDayOfWeek(day: number): string {
    const date = new Date();
    const dayOfWeek = dateFns.format(dateFns.setDay(date, day, { weekStartsOn: 1 }), 'dd', {
      locale: locales[this.options.locale]
    });
  ...
  toggleCalendar(e: MouseEvent, selection: 'from' | 'to'): void {
    ...
    this.calendarToggleEvent.emit(!!this.opened);
  }

  closeCalendar(e: MouseEvent): void {
    this.opened = false;
    this.calendarToggleEvent.emit(false);
  }
